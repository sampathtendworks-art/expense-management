...
# --- CONTRACT SCHEMAS (Validated & Defaulted) ---
class OCRResultsSchema(BaseModel):
    ocr_confidence: float = Field(1.0, description="Average confidence score from OCR service")
...
class ReconResultsSchema(BaseModel):
    status: str = Field("Unverified", description="Reconciliation status (e.g. VERIFIED, MISMATCH)")
    mismatch_amount: float = Field(0.0, description="Discrepancy amount between bank and claim")
...
class EvaluationInputSchema(BaseModel):
    ...
    recon_results: ReconResultsSchema = ReconResultsSchema()
