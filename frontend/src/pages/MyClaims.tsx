...
const STATUS_CONFIG = {
  ...
  under_review: { bg: 'bg-amber-50', border: 'border-[#F59E0B]/30', text: 'text-[#F59E0B]', icon: History, label: 'Under Review' },
  approved: { bg: 'bg-emerald-50', border: 'border-[#22C55E]/30', text: 'text-[#22C55E]', icon: CheckCircle2, label: 'Approved' },
  rejected: { bg: 'bg-rose-50', border: 'border-[#EF4444]/30', text: 'text-[#EF4444]', icon: XCircle, label: 'Rejected' },
  fast_track: { bg: 'bg-emerald-50', border: 'border-[#22C55E]/30', text: 'text-[#22C55E]', icon: CheckCircle2, label: 'Fast-Track' },
  manager_review: { bg: 'bg-amber-50', border: 'border-[#F59E0B]/30', text: 'text-[#F59E0B]', icon: History, label: 'Manager Review' },
  escalated: { bg: 'bg-rose-50', border: 'border-rose-100', text: 'text-rose-700', icon: AlertCircle, label: 'Finance Escalation' },
  paid: { bg: 'bg-teal-50', border: 'border-teal-150', text: 'text-teal-700', icon: CheckCircle2, label: 'Paid' },
  pending: { bg: 'bg-amber-50', border: 'border-[#F59E0B]/30', text: 'text-[#F59E0B]', icon: History, label: 'Pending' },
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-10">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-black text-primary tracking-tight uppercase">My Claims</h2>
          <p className="text-slate-500 mt-2 font-medium">Manage and track your expense submissions</p>
        </div>
            <button 
              onClick={() => navigate('/new-claim')}
              className="flex items-center gap-2 px-6 py-3 bg-accent text-white rounded-xl text-xs font-black hover:bg-emerald-600 transition-all shadow-xl shadow-accent/20 uppercase tracking-widest cursor-pointer"
            >
              [ + New Expense ]
            </button>
      </div>
        ...
        <div className="flex items-center gap-6 flex-1 max-w-2xl">
          ...
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:outline-none focus:border-accent transition-all"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <div className="flex items-center gap-1.5 p-1.5 bg-slate-50 rounded-xl border border-slate-200 overflow-x-auto">
            {['all', 'submitted', 'manager_review', 'fast_track', 'approved', 'rejected', 'escalated', 'paid'].map((status) => (
              <button
                key={status}
